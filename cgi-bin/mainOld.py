...
def computeX2(attrVals, clusters, data, N):
	...
	for j in range(M):
		...
		for p in range(k):
			...
			otherObjs = numpy.setdiff1d(list(range(N)), clusterObjs)
...
for p in range(k):
	...
	temp = temp[::-1]
	keyterms.append(temp[list(range(f))])
...
print(json.dumps({'termClusters':output1, 'documentClusters':output2, 'silhouette':output3}))
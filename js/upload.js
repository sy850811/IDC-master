...
function checkUserExists() {
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (status == "no") {
        console.log("hello from upload.js checkUserExist");
  ...
  $.ajax({
        ...
        $.ajax({
          ...
          success: function (msg) {
            ...
            if (status == "yes") {
              ...
              $("#upload_output").append(
                "<tr><th align='left'style='width:80%;'>" +
                  file_name +
                  "</th><th align='left' style='color:green;width:20%;'> Uploaded successfully!</th></tr>"
              );
              ...
              $("#upload_output").append(
                "<tr><th align='left'style='width:80%;'>" +
                  file_name +
                  "</th><th align='left' style='color:red;width:20%;'>" +
                  errorMessage +
                  "</th></tr>"
              );
              $("#upload_output").scrollTop($("#upload_output").height());
          ...
          error: function (msg) {
            $("#upload_output").append(
              "<tr><th align='left'style='width:80%;'>" +
                file_name +
                "</th><th align='left' style='color:red;width:20%;'Error Saving file!</th></tr>"
            );
            $("#upload_output").scrollTop($("#upload_output").height());
          },
        });
    ...
    error: function (msg) {
      $("#upload_output").append(
        "<tr><th align='left'style='width:80%;'>" +
          file_name +
          "</th><th align='left' style='color:red;width:20%;'>Error Uploading!</th></tr>"
      );
      $("#upload_output").scrollTop($("#upload_output").height());
    },
  });
      ...
      if (files[i].type == "application/pdf") {
        if (files[i].size > 52428800) {
          $("#upload_output").append(
            "<tr><th align='left'style='width:80%;'>" +
              files[i].name +
              "</th><th align='left' style='color:red;width:20%;'> Large file (>50MB)!</th></tr>"
          );
          $("#upload_output").scrollTop($("#upload_output").height());
          loaded++;
        } else {
        ...
        $("#upload_output").append(
          "<tr><th align='left'style='width:80%;'>" +
            files[i].name +
            "</th><th align='left' style='color:red;width:20%;'> Not supported format!</th></tr>"
        );
        $("#upload_output").scrollTop($("#upload_output").height());
        loaded++;
      }
...
function CSV_Upload() {
  $("#button_csv").change(function (e) {
    ...
    fileReader.onload = function (fileLoadedEvent) {
      csvFile = d3.csvParse(fileLoadedEvent.target.result);

      //show the columns to the user
      var columns = Object.keys(csvFile[0]);

      $("#upload_output").append(
        "<p><strong>Please select columns that should be in final PDF files and then press 'Submit'.</strong></p>"
      );

      for (var i = 0; i < columns.length; i++) {
        if (columns[i].length > 0) {
          $("#upload_output").append(
            "<input type='checkbox' class='column' value='" +
              columns[i] +
              "'>" +
              columns[i] +
              "<br>"
      ...
      $("#upload_output").append(
        "<br><input id='CSVsubmit' title='Submit' type='button' value='Submit' onmousedown='converToPDF()' />"
      );
    };
...
function converToPDF() {
  ...
  for (var i = 0; i < csvFile.length; i++) {
    ...
    for (var j = 0; j < keys.length; j++) {
      //for each row selec only content of the selected columns
...
function createPDF() {
  $.ajax({
    ...
    data: {
      userDirectory: JSON.stringify(userDirectory),
      TXT_Documents: JSON.stringify(TXT_Documents),
    },
    async: false,
    success: function (msg) {
      var status = msg["status"];
      ...
      if (status == "yes") {
        $("#upload_output").append(
          "<p id='r1'><strong>The documents extracted and saved successfully. You can also download the PDF files by pressing the 'Download' button.</strong></p>"
        );
        $("#upload_output").append(
          "<input id='downloadPDF' title='Download' type='button' value='Download' onclick='downloadPDF()'/>"
        );
      } else {
...
function deleteAll() {
  if (userID == "") {
    ...
    deleteConfirm = confirm(
      "Are you sure about removing all files in your directory?"
    );
...
function generateFilesList(files) {
  $("#file_list_output").html("");
  for (var i = 0; i < files.length; i++) {
    $("#file_list_output").append(
      "<input style='margin-left:1%;' type='checkbox' class='fileListCheckBox' value='" +
        files[i] +
        "'>" +
...
function preprocess() {
  ...
  document.body.style.cursor = "progress";
  $("#UploadprogressBar").html(
    "<p>Please Wait while the dataset is processing!</p>"
  );
  ...
  $.ajax({
    ...
    error: function (a) {
      if (a.status == 504) {
        alert(
          "Time out (error 504), Preprocessing of documents is not finnished. Check the files from the server."
        );
        document.body.style.cursor = "auto";
        $("#UploadprogressBar").html("");
      } else {
